...
useEffect(() => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Using the endpoint relative to the base URL configured in your AuthContext api instance
        const response = await api.get("/cdpo/demand-distribution-mp/");
  ...
  // Filter data based on selected financial year and quarter
        ...
        <Container fluid className="dashboard-box mt-3">
          <div className="main-heading">
            <h3 className="mb-0 fw-bold">
              Demand & Distribution (Mahila Poshan)
            </h3>
          ...
          {loading ? (
            <div className="text-center py-5">
              ...
              <p className="mt-2">Loading demand records...</p>
            </div>
          ) : error ? (
            <div className="alert alert-danger">{error}</div>
          ) : demandData.length === 0 ? (
            <div className="text-center py-5 text-muted">No demand records found.</div>
          ) : (
              ...
              <Table hover bordered className="align-middle text-center mb-0" style={{ fontSize: '11px' }}>
                <thead>
                  <tr>
                    ...
                    <th colSpan="3" className="bg-success text-white">Khajur Distribution</th>
                    <th colSpan="3" className="bg-danger text-white">Egg Distribution</th>
                    <th colSpan="3" className="bg-warning text-dark">Veg / Not-Eat-Egg</th>
                  </tr>
                  ...
                  {filteredData.length > 0 ? (
                    filteredData.map((demand) => (
                      demand.distribution.map((dist, dIdx) => (
                        <tr key={`${demand.demand_id}-${dist.id}`} className="table-light">
                          {dIdx === 0 ? (
                            <>
                              ...
                              <td rowSpan={demand.distribution.length}>
                                <Badge bg={demand.cdpo_status === "Approve" ? "success" : "warning"}>
                                  {demand.cdpo_status}
                                </Badge>
                              </td>
                            </>
                          ...
                          <td>{dist.tot_bene}</td>
                          {/* Khajur Distribution */}
                          <td>{demand.khajur_beneficiary}</td>
                          <td>{dist.khajur_disti_bene}</td>
                          <td className="fw-bold text-success">{dist.khajur_disti}</td>
                          {/* Egg Distribution */}
                          <td>{demand.egg_beneficiary}</td>
                          <td>{dist.egg_disti_bene}</td>
                          <td className="fw-bold text-danger">{dist.egg_disti}</td>
                          {/* Veg / Not-Eat-Egg Distribution */}
                          <td>{demand.total_not_eat_egg_beneficiary}</td>
                          <td>{dist.tot_noteat_egg_disti_bene}</td>
                          <td className="fw-bold text-dark">{dist.tot_noteat_egg_disti}</td>
                        </tr>
                      ))
                    ))
                  ) : (
                    <tr>
                      <td colSpan="16" className="text-center py-4">No records found for the selected year.</td>
                    </tr>
                  )}
                </tbody>
              </Table>
