...
const DemandBalPoshanProject = () => {
  ...
  const columns = [
    ...
    { key: "old_bal", label: "Old Bal" },
    { key: "kela", label: "Banana Bene" },
    { key: "egg", label: "Egg Bene" },
    { key: "non_egg", label: "Not Consume Egg Bene" },
    { key: "cdpo_status", label: "CDPO Status" },
    { key: "dir_status", label: "DPO Status" },
  ];
  ...
  const handleCopy = async () => {
    ...
    const mRows = pendingData.map((item, idx) => {
      ...
      if (visibleColumns.old_bal) row.push(item.old_bal || "0");
    ...
    const text = "Bal Poshan Project Demand Report\n" + [mHeaders.join("\t"), ...mRows].join("\n");
    try {
  ...
  const handleExcel = () => {
    const mHeaders = columns.filter(c => visibleColumns[c.key]).map(c => c.label);
    let csv = "Bal Poshan Project Demand Report\n" + mHeaders.join(",") + "\n";
    ...
    link.download = "Bal_Poshan_Project_Demand_Report.csv";
    link.click();
  };
  ...
  const handlePDF = () => {
    ...
    const mRows = pendingData.length > 0 ? pendingData.map((item, idx) => {
      ...
      columns.filter(c => visibleColumns[c.key]).forEach(col => {
        ...
        else if (col.key === "old_bal") val = item.old_bal;
        else if (col.key === "kela") val = item.kela_chips_bene ?? 0;
        else if (col.key === "egg") val = item.egg_bene ?? 0;
        else if (col.key === "non_egg") val = item.not_eat_egg_bene ?? 0;
      ...
      row += "</tr>";
      return row;
    }).join("") : `<tr><td colspan="${columns.filter(c => visibleColumns[c.key]).length}" style="text-align:center;">No data available</td></tr>`;
    printWindow.document.write(`
      <html>
        ...
        <body>
          <h2 style="text-align:center">Bal Poshan Project Demand Report</h2>
  ...
  const fetchData = async () => {
    ...
    setError("");
    try {
      const response = await api.get("/cdpo-bp-demand/");
  ...
  const renderDemandTable = () => {
    ...
    return pendingData.map((item, index) => (
      <tr key={item.id}>
        ...
        {visibleColumns.old_bal && <td>{item.old_bal || "0"}</td>}
  ...
  const renderApprovalTable = () => {
    ...
    if (approvedData.length === 0) {
      return (
        <tr>
          <td colSpan="13" className="text-center py-4 text-muted">No approved records found</td>
        </tr>
      );
    }
  ...
  return (
    <div className="dashboard-container">
        ...
        <Container fluid className="dashboard-box mt-3">
          <div className="main-heading">
            <h3 className="mb-4 fw-bold">
              मुख्यमंत्री बाल पोषण योजना अवलोकन(डिमांड पैनल)
            </h3>
          </div>
                ...
                <Table striped bordered hover size="sm" className="mb-0" ref={tableRef}>
                  <thead className="table-light">
                    <tr>
                      ...
                      {visibleColumns.old_bal && <th>Old Bal</th>}
                      {visibleColumns.kela && <th>Banana Bene</th>}
                      {visibleColumns.egg && <th>Egg Bene</th>}
                      {visibleColumns.non_egg && <th>Not Consume Egg Bene</th>}
      ...
      <Modal show={showColumnModal} onHide={() => setShowColumnModal(false)} size="sm" centered>
        ...
        <Modal.Body>
          <Form.Check 
            type="checkbox" label="Select All" className="mb-2 fw-bold border-bottom pb-2"
...
export default DemandBalPoshanProject;
