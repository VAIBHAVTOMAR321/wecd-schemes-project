...
const DirDemandAnchalSec = () => {
  ...
  const tableColumns = [
    ...
    { key: "available_month", label: "Available Month" },
  ...
  const fetchDemandData = useCallback(async () => {
    ...
    try {
      ...
      const mappedData = fetchedData.map(item => ({
        ...
        available_month: item.available_month || "",
      ...
      console.error("Error fetching amrit anchal sector demand data:", err);
    ...
    sortedData.forEach((row, index) => {
        ...
        if (currentDistrict !== null && currentProject !== null) {
          rows.push(
            <tr key={`subtotal-${currentDistrict}-${currentProject}`} style={{ backgroundColor: "#e8f4f8", fontWeight: "bold" }}>
              ...
              {visibleColumns.available_month && <td></td>}
              {visibleColumns.milk_beneficiary && <td className="text-center">{projectSubtotal.milkBeneficiary}</td>}
            </tr>
          );
      ...
      rows.push(
        <tr key={row.id || index}>
          ...
          {visibleColumns.available_month && <td className="text-center">{row.available_month}</td>}
  ...
  const handleExcel = () => {
    ...
    link.href = url;
    link.download = `Amrit_Anchal_Sector_Report_${financialYear || "All"}.csv`;
    link.click();
  ...
  const handlePDF = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Amrit Anchal Sector Report</title>
          <style>
              ...
              <Table striped bordered hover size="sm" className="mb-0" ref={tableRef}>
                <thead style={{ backgroundColor: "#004d4d", color: "#fff" }}>
                  <tr className="text-center">
                    ...
                    {visibleColumns.available_month && <th className="py-2">Available Month</th>}
...
export default DirDemandAnchalSec;
