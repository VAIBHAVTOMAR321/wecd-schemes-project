...
import { 
  FaBuilding, FaMedal, FaSearch, FaCopy, FaFileExcel, FaFilePdf, FaEye, 
...
const OurDpo = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await api.get("https://mahadevaaya.com/wecdschemes/wecdschemes_backend/api/director/districts/");
  ...
  const handleCopy = async () => {
    const headers = ["S.no", "District", "District Code", "DPO Incharge", "DPO Mobile", "Status"];
    const rows = paginatedData.map((item, idx) => [
      (currentPage - 1) * itemsPerPage + idx + 1,
      item.district, item.code, "-", "-", item.stat_fin
    ]);
  ...
  const handleExcel = () => {
    ...
    paginatedData.forEach((item, idx) => {
      const row = [(currentPage - 1) * itemsPerPage + idx + 1, item.district, item.code, "-", "-", item.stat_fin];
      csv += row.join(",") + "\n";
    });
    const blob = new Blob([csv], { type: "text/csv" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = "DPO_Districts_Report.csv";
    link.click();
  };
  ...
  const handlePDF = () => {
    ...
    printWindow.document.write(`
      <html>
        <head><title>DPO Districts Report</title><style>
        ...
        <body>
          <h2 style="text-align:center">District | DPO Incharge Report</h2>
        ...
        <Container fluid className="p-4 bg-white" style={{ minHeight: '100vh' }}>
          <div className="d-flex align-items-center mb-4">
            <div className="p-2 rounded me-3 d-flex align-items-center justify-content-center" style={{ backgroundColor: '#e0f2f1', width: '40px', height: '40px' }}>
              ...
              <FaMedal style={{ color: '#14b8a6', marginLeft: '-4px', marginTop: '4px' }} size={12} />
            </div>
            <h4 className="fw-bold text-dark m-0" style={{ letterSpacing: '-0.5px' }}>District | DPO Incharge</h4>
          </div>
              ...
              <Table hover className="align-middle border-top" style={{ fontSize: '13px' }} ref={tableRef}>
                <thead className="bg-white">
                  <tr style={{ borderBottom: '2px solid #f1f5f9' }}>
                    ...
                    {visibleColumns.code && <th className="py-3 text-muted" onClick={() => handleSort('code')} style={{ cursor: 'pointer' }}>District Code <SortIcon colKey="code" /></th>}
                    {visibleColumns.dpo_incharge && <th className="py-3 text-muted">DPO Incharge</th>}
                    {visibleColumns.dpo_mobile && <th className="py-3 text-muted">DPO Mobile</th>}
                ...
                <tbody style={{ backgroundColor: '#f8fafc' }}>
                  {paginatedData.map((item, idx) => (
                    <tr key={idx} style={{ backgroundColor: idx % 2 === 0 ? '#f8fafc' : '#ffffff', borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      {visibleColumns.dpo_incharge && <td>-</td>}
                      {visibleColumns.dpo_mobile && <td>-</td>}
