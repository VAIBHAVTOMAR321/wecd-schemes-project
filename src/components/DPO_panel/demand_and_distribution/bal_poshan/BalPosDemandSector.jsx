  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await api.get("/bal-poshan-sector-wise/", { params });
        if (cancelled) return;
        const payload = response.data;
        const projects = Array.isArray(payload?.data) ? payload.data : [];
        const flattenedSectors = projects.flatMap((project) => {
          const sectors = Array.isArray(project?.sectors) ? project.sectors : [];
          return sectors.map((sector) => ({
            ...sector,
            project_name: project.project_name || "",
            total_sectors: project.total_sectors || null,
          }));
        });

        setDistrict(payload?.district || "");
        setTotalProjects(payload?.total_projects ?? null);
        setData(flattenedSectors);
      } catch (err) {
        if (!cancelled) {
          console.error("Failed to fetch Bal Poshan sector-wise data:", err);
          setData([]);
          setDistrict("");
      ...
      <div className="main-content-dash">
        ...
        <Container fluid className="dashboard-box mt-3">
          <div className="main-heading">
            <h3 className="mb-2 fw-bold">District Wise Stock Demand : Anchal Amrit Yojana</h3>
          </div>
            ...
            <Col md={3}>
              <Card className="border-0 shadow-sm stats-card h-100">
                <Card.Body className="text-center">
                  <Card.Title className="stats-title">Total Projects</Card.Title>
                  <Card.Text className="stats-count">
                    {typeof totalProjects === "number" ? totalProjects : "-"}
                  </Card.Text>
                </Card.Body>
              </Card>
            </Col>
...
export default BalPosDemandSector;
