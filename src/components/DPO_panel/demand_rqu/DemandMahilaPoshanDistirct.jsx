...
const DemandMahilaPoshanDistirct = () => {
  ...
  const handleCopy = async () => {
    ...
    const text = "Mahila Poshan District Demand Report\n" + [mHeaders.join("\t"), ...mRows].join("\n");
    try {
  ...
  const handleExcel = () => {
    if (filteredData.length === 0) return;
    const mHeaders = columns.filter(c => visibleColumns[c.key]).map(c => c.label);
    let csv = "Mahila Poshan District Demand Report\n" + mHeaders.join(",") + "\n";
    ...
    link.download = "Mahila_Poshan_District_Demand_Report.csv";
    link.click();
  };
  ...
  const handlePDF = () => {
    ...
    const mRows = filteredData.map((item, idx) => {
      ...
      if (visibleColumns.khajur) row += `<td>${item.khajur_bene ?? 0}</td>`;
      if (visibleColumns.egg) row += `<td>${item.egg_bene ?? 0}</td>`;
      if (visibleColumns.non_egg) row += `<td>${item.tot_noteat_egg_bene ?? 0}</td>`;
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          <h2>Mahila Poshan District Demand Report</h2>
